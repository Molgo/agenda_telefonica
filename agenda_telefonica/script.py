...
while True:
    ...
    if r == "1":
        ...
        while int(contato_selecionado) - 1 not in range(len(lista_contatos)):
            contato_selecionado = input("Digite o número do contato que deseja editar: ")
        novo_nome_contato =  input(f"Novo nome para o contato {contato_selecionado}: ")
        novo_telefone = input(f"Novo telefone para o contato {contato_selecionado}: ")
        novo_email = input(f"Novo email para o contato {contato_selecionado}: ")
        editar_contato(lista_contatos, contato_selecionado, novo_nome_contato, novo_telefone, novo_email)
